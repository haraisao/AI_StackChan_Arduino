...
### リポジトリのダウンロード
PlatformIOの準備が整ったら、gitコマンドでリポジトリのソースコードをダウンロードします。
...
アクセスポイントになった場合には、「M5StackAP」という名前で起動し、パスワードは、「stack-chan」になっています。アクセスポイント名とパスワードの変更は、Utils.cppを直接変更してください。
